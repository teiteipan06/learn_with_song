...
# 工夫したポイント
登録した曲は、曲一覧に表示される際に、ユーザーにとって、より分かりやすくするために、どうしても曲動画のサムネイルも見えるようにさせたかったです。色々調べたところで、実装の方法はいくつも見つけましたが、どのやり方でも、YoutubeのAPIを使う方法でした。

私はやりたいのは単純にサムネイルの表示させるだけで、そんなに手間が掛からなくても、他にもっといい方法があるではないかと思いました。そして、サムネイルについて調べて、色んな方法を試しました。

何回も失敗して、そこで、気づいたのは違うYoutubeの動画でも、サムネイルのURLは一部だけ変わるので、その部分だけ変数として使いましたら
、簡単なHTMLだけでもサムネイルが表示できるようになりました。
